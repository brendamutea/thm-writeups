# 🔐 TryHackMe Writeups — Brenda Mutea

A documented learning journal of my TryHackMe journey as I transition 
from reinsurance into cybersecurity. Each writeup reflects my own thinking, 
mistakes, and takeaways — not just the answers.

> "The obstacle is the path." — every cybersecurity beginner, ever.

---

## 📊 Progress Tracker

| Path | Rooms Completed | Status |
|------|----------------|--------|
| Cyber Security 101 | 3 / 14 | 🔄 In Progress |
| SOC Level 1 | 0 / 26 | ⏳ Not Started |
| Jr Penetration Tester | 0 / 28 | ⏳ Not Started |

---

## 📁 Writeups Index

### 🟢 Cyber Security 101 Path
*Writeups will appear here as I complete each room.*

---

### 🔵 SOC Level 1
*Coming soon.*

---

### 🔴 Jr Penetration Tester
*Coming soon.*

---

## 🧠 What I Include in Each Writeup

- **My approach** — how I tackled the room in my own words
- **Commands used** — with explanations of *why*, not just *what*
- **What confused me** — honest reflection on sticking points
- **Key takeaways** — what I'll actually remember
- **Real-world connection** — how this applies to a real security job

---

## 🛠 Tools Used Across These Labs
![Nmap](https://img.shields.io/badge/Nmap-blue)
![Wireshark](https://img.shields.io/badge/Wireshark-blue)
![Linux](https://img.shields.io/badge/Linux-black)
![Python](https://img.shields.io/badge/Python-yellow)
![Burp Suite](https://img.shields.io/badge/BurpSuite-orange)

---

## 📅 Started: March 2026
*Updated regularly as I progress through each path.*
